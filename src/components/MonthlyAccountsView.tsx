...
export default function MonthlyAccountsView({
  ...
  const statusConta = (conta: ContaMensal) => {
    if (!conta.ativo) return { label: 'Inativa', bg: '#EDF0EE', color: '#818885', icon: 'x' }
    if (contasPagas.has(conta.id)) return { label: 'Pago neste mês', bg: '#E8F0EB', color: ACCENT_LT, icon: 'check' }
    if (hoje.getDate() > conta.dia_vencimento) return { label: 'Em atraso', bg: '#F1F4F2', color: '#606463', icon: 'alert' }
    return { label: `Vence dia ${conta.dia_vencimento}`, bg: '#E8EFEB', color: ACCENT_LT, icon: 'calendar' }
  }
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',alignItems:'flex-start',gap:13,background:'#E8EFEB',border:'1px solid #D6E2DB',borderRadius:14,padding:'14px 16px',marginBottom:'1.25rem'}}>
        ...
        <div>
          <p style={{fontSize:12,fontWeight:800,color:'#606463',margin:'1px 0 4px'}}>Como acompanhar</p>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:12,marginBottom:'1.35rem'}}>
        ...
        <KPI l="Pagas neste mês" v={contasPagas.size} sv={`de ${ativas.length} contas ativas`} c="#7F9C93"/>
        <KPI l="Total pago no mês" v={fmtR(totalPagoMes)} sv={mesAtualNome} c="#6F8E83"/>
        <KPI l="Precisam de atenção" v={emAtraso} sv="vencidas sem pagamento" c="#818885"/>
      </div>

      <div style={s.card}>
        <div style={{...s.toolbar,justifyContent:'space-between'}}>
          <div>
            <p style={{fontSize:12,fontWeight:800,color:'#606463',margin:0}}>Acompanhamento de {mesAtualNome}</p>
            <p style={{fontSize:11,color:'#929A96',margin:'3px 0 0'}}>Clique em uma conta para abrir o histórico completo.</p>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap'}}>
            <div style={{display:'flex',gap:3,padding:3,border:'1px solid #E1E7E3',borderRadius:9,background:'#F6F8F7'}}>
              <button onClick={()=>setViewMensal('lista')} style={{padding:'.4rem .75rem',borderRadius:7,border:'none',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',background:viewMensal==='lista'?ACCENT:'transparent',color:viewMensal==='lista'?'#fff':'#818885'}}>Lista</button>
              <button onClick={()=>setViewMensal('grade')} style={{padding:'.4rem .75rem',borderRadius:7,border:'none',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',background:viewMensal==='grade'?ACCENT:'transparent',color:viewMensal==='grade'?'#fff':'#818885'}}>Grade anual</button>
            </div>
        ...
        {viewMensal === 'lista' ? (
          <div style={{padding:'0 1.2rem 1.2rem'}}>
            <div style={{display:'grid',gridTemplateColumns:'minmax(190px,1.35fr) minmax(120px,.8fr) minmax(150px,1fr) minmax(135px,.9fr) minmax(190px,1.25fr)',gap:12,padding:'12px 10px 9px',borderBottom:'1px solid #E1E7E3',color:'#929A96',fontSize:10,fontWeight:800,letterSpacing:'.10em',textTransform:'uppercase'}}>
            ...
            {filtered.length === 0 ? (
              <div style={{textAlign:'center',padding:'3.2rem 1rem'}}>
                <span style={{width:44,height:44,borderRadius:13,background:'#E8EFEB',color:ACCENT_LT,display:'inline-flex',alignItems:'center',justifyContent:'center'}}><Icon name="refresh" size={21}/></span>
                <p style={{fontSize:15,fontWeight:800,color:'#606463',margin:'13px 0 5px'}}>{contasMensais.length ? 'Nenhuma conta encontrada' : 'Comece pelas suas contas fixas'}</p>
                <p style={{fontSize:12,color:'#929A96',maxWidth:330,lineHeight:1.5,margin:'0 auto 14px'}}>{contasMensais.length ? 'Tente outro nome na busca.' : 'Cadastre água, luz, internet ou qualquer despesa recorrente para acompanhar todos os meses.'}</p>
              ...
              return (
                <div key={conta.id} onClick={()=>onHistorico(conta)} style={{display:'grid',gridTemplateColumns:'minmax(190px,1.35fr) minmax(120px,.8fr) minmax(150px,1fr) minmax(135px,.9fr) minmax(190px,1.25fr)',gap:12,alignItems:'center',padding:'14px 10px',borderBottom:'1px solid #E1E7E3',cursor:'pointer',transition:'background .15s'}} onMouseEnter={e=>(e.currentTarget.style.background='#F4F7F5')} onMouseLeave={e=>(e.currentTarget.style.background='')}>
                  <div style={{minWidth:0}}>
                    <p style={{fontSize:13,fontWeight:800,color:'#606463',margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{conta.titulo}</p>
                    <p style={{fontSize:11,color:'#929A96',margin:'4px 0 0'}}>Pago por {conta.pago_por}</p>
                  </div>
                  <div>
                    <p style={{fontSize:12,fontWeight:700,color:'#606463',margin:0}}>Dia {conta.dia_vencimento}</p>
                    <p style={{fontSize:10,color:'#929A96',margin:'3px 0 0'}}>recorrente mensal</p>
                  </div>
                  <Badge label={status.label} bg={status.bg} color={status.color}/>
                  <div>
                    {pagoAtual !== null ? <p style={{fontSize:12,fontWeight:800,color:ACCENT_LT,margin:0}}>{fmtR(pagoAtual)}</p> : <p style={{fontSize:12,fontWeight:700,color:'#929A96',margin:0}}>{ultimo ? fmtR(ultimo.valor) : 'Ainda não pago'}</p>}
                    <p style={{fontSize:10,color:'#929A96',margin:'3px 0 0'}}>{pagoAtual !== null ? 'pago neste mês' : ultimo ? `em ${fmtData(ultimo.data_pagamento)}` : 'sem histórico'}</p>
                  </div>
                  <div style={{display:'flex',justifyContent:'flex-end',gap:6,flexWrap:'wrap'}} onClick={e=>e.stopPropagation()}>
                    ...
                    <button title={conta.ativo?'Pausar conta':'Reativar conta'} onClick={()=>api.toggleContaMensal(conta.id,!conta.ativo).then(onAtualizar)} style={{width:30,height:30,display:'inline-flex',alignItems:'center',justifyContent:'center',background:'#fff',border:'1px solid #E1E7E3',borderRadius:8,cursor:'pointer'}}><Icon name={conta.ativo?'x':'check'} size={14} color={conta.ativo?'#777B79':ACCENT_LT}/></button>
                  </div>
                </div>
              )
            })}
            {filtered.length > 0 && <div style={{padding:'13px 10px 0',fontSize:11,color:'#929A96'}}>{filtered.length} conta{filtered.length !== 1 ? 's' : ''} exibida{filtered.length !== 1 ? 's' : ''} · {contasPagas.size} paga{contasPagas.size !== 1 ? 's' : ''} neste mês</div>}
          </div>
        ) : (
          <div style={{padding:'1.1rem 1.2rem 1.35rem'}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,fontSize:11,color:'#818885'}}><span style={{width:9,height:9,borderRadius:3,background:'#E8F0EB'}}/> Pago <span style={{width:9,height:9,borderRadius:3,background:'#F1F4F2',marginLeft:6}}/> Pendente <span style={{marginLeft:'auto'}}>Clique em uma célula pendente para registrar</span></div>
            {filtered.length === 0 ? <p style={{textAlign:'center',padding:'2.5rem',color:'#929A96',fontSize:13}}>Nenhuma conta encontrada.</p> : (
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:830}}>
                  <thead><tr>
                    <th style={{padding:'9px 10px',textAlign:'left',fontSize:10,fontWeight:800,color:'#929A96',textTransform:'uppercase',position:'sticky',left:0,background:'#fff',zIndex:1}}>Conta</th>
                    {meses.map(m=><th key={m.key} style={{padding:'9px 5px',textAlign:'center',fontSize:10,fontWeight:800,color:'#929A96',textTransform:'uppercase',whiteSpace:'nowrap'}}>{m.label}</th>)}
                  </tr></thead>
                  <tbody>{filtered.map(conta=><tr key={conta.id}>
                    <td style={{padding:'10px',fontWeight:800,color:'#606463',whiteSpace:'nowrap',position:'sticky',left:0,background:'#fff',zIndex:1}}>{conta.titulo}<span style={{display:'block',fontSize:10,fontWeight:500,color:'#A2AAA6',marginTop:3}}>dia {conta.dia_vencimento}</span></td>
                    {meses.map(m=>{
