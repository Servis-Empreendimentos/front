...
export function KPI({l,v,sv,c}:{l:string;v:string|number;sv?:string;c:string}) {
  return (
    <div style={s.kpi}>
      ...
      <p style={{fontSize:10,fontWeight:600,color:'#7D7D7D',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6}}>{l}</p>
      <p style={{fontSize:22,fontWeight:700,color:'#626262',lineHeight:1.1}}>{v}</p>
      {sv&&<p style={{fontSize:11,color:'#7D7D7D',marginTop:4}}>{sv}</p>}
    </div>
  )
}
...
export function AnexoBtn({url,label,icon,onAnexar,onSubstituir,loading}:{url?:string|null;label:string;icon:string;onAnexar:()=>void;onSubstituir:()=>void;loading:boolean}) {
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:12}}>
      <span style={{fontSize:12,color:'#7D7D7D'}}>Sem {label} anexada</span>
...
export function FornecedorInput({value,cnpj,onChange}:{value:string;cnpj:string;onChange:(n:string,c:string)=>void}) {
  ...
  return (
    <div style={{position:'relative' as const}}>
      ...
      {aberto&&(
        <div style={{position:'absolute',top:'100%',left:0,right:0,background:'#fff',border:'1.5px solid #E2E6E4',borderRadius:8,zIndex:100,boxShadow:'0 4px 16px rgba(0,0,0,.1)',maxHeight:200,overflowY:'auto'}}>
          {sugestoes.map(f=>(
            <div key={f.id} onClick={()=>{onChange(f.nome,f.cnpj||'');setAberto(false)}}
              ...
              {f.cnpj&&<p style={{margin:0,fontSize:11,color:'#7D7D7D'}}>{fmtCNPJ(f.cnpj)}</p>}
            </div>
          ))}
        </div>
      )}
...
export function ItensEditor({itens,onChange}:{itens:ItemLancamento[];onChange:(i:ItemLancamento[])=>void}) {
  ...
  return (
    <div style={{gridColumn:'1/-1',border:'1.5px solid #E2E6E4',borderRadius:8,overflow:'hidden'}}>
      <div style={{background:'#FAFBFA',padding:'8px 12px',borderBottom:'1px solid #E2E6E4',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <span style={{fontSize:10,fontWeight:700,color:'#7D7D7D',textTransform:'uppercase',letterSpacing:'.05em'}}>Itens do orçamento ({itens.length}) — opcional</span>
      ...
      {itens.map((item,i)=>(
        <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 70px 110px 110px 24px',gap:8,padding:'8px 12px',borderBottom:'1px solid #E2E6E4',alignItems:'end'}}>
          ...
          <div>
            ...
            <input style={{...s.fi,fontSize:12,background:'#F6F8F7',color:'#8BA59A',fontWeight:700}} value={fmtR(item.valor_total||0)} readOnly/>
          </div>
          <button onClick={()=>rem(i)} type="button" style={{background:'none',border:'none',cursor:'pointer',color:'#777777',padding:0,display:'flex'}}><Icon name="x" size={16}/></button>
        </div>
      ))}
      {itens.length>0&&(
        <div style={{padding:'8px 12px',background:'#F6F8F7',display:'flex',justifyContent:'flex-end'}}>
          <span style={{fontSize:13,fontWeight:700,color:'#8BA59A'}}>Total: {fmtR(total)}</span>
        </div>
      )}
    </div>
  )
            ...
            <div key={step.id} style={{display:'flex',alignItems:'center',flex:1}}>
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4,flex:'0 0 auto'}}>
                <button onClick={()=>onChange(step.id)} style={{
                  ...
                  background:done?'#8BA59A':current?cor:'#E2E6E4',
                  display:'flex',alignItems:'center',justifyContent:'center',
                  boxShadow:current?`0 0 0 3px ${cor}33`:'none',transition:'all .2s',
                }}>
                  {done?<Icon name="check" size={16} color="#fff"/>:<Icon name={STEP_ICONS[step.id]} size={16} color={current?'#fff':'#7D7D7D'}/>}
                </button>
                <span style={{fontSize:9,fontWeight:600,color:current?cor:done?'#8BA59A':'#7D7D7D',textAlign:'center',maxWidth:70,lineHeight:1.2}}>{step.label}</span>
              </div>
              {i<PIPELINE.length-1&&<div style={{flex:1,height:2,background:i<idx?'#8BA59A':'#E2E6E4',margin:'0 4px',marginBottom:20}}/>}
            </div>
          )
        })}
      </div>
