...
export default function Home() {
  ...
  const [role, setRole]         = useState<'lancadora'|'gestora'>('lancadora')
  ...
  const load = useCallback(async()=>{
    ...
    try {
      const [lista, categorias] = await Promise.all([
        api.listar({ status_entrega:fStatus, tipo_pagamento:fTipo, categoria_id:fCat }),
        api.categorias(),
      ])
  ...
  const openNovo = () => {
    setForm({ tipo_pagamento:'avista', data:new Date().toISOString().slice(0,10) })
    setParcelas([]); setDetalhe(null); setModal(true)
  }

  const openDetalhe = async(id:string) => {
    const d = await api.buscar(id)
    setDetalhe(d); setParcelas(d.parcelas||[]); setModal(true)
  }

  const handleSave = async() => {
    if (!form.titulo||!form.valor_total||!form.data||!form.categoria_id||!form.pago_por)
      return showToast('Preencha todos os campos obrigatórios',false)
    setSaving(true)
    try {
      await api.criar({ ...form, valor_total:parseFloat(form.valor_total), criado_por:user, parcelas })
    ...
    <div style={s.page}>
      <header style={s.topbar}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'#7A919E' }}>
          <span>👤 {user}</span>
          <span style={{ ...s.badge, background:role==='gestora'?'#EAF7EE':'#E0F5F7', color:role==='gestora'?'#27AE60':'#0097A8' }}>
            {role==='gestora'?'Gestora':'Lançadora'}
          </span>
      ...
      <main style={s.main}>
        <div style={s.row}>
          <div>
            <h1 style={s.h1}>Lançamentos — Notas Fiscais</h1>
            <p style={s.p}>Controle de pagamentos e entregas · Financeiro</p>
          </div>
        ...
        <div style={s.card}>
          ...
          <div style={{ overflowX:'auto', maxHeight:440, overflowY:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              ...
              <tbody>
                {loading
                  ? <tr><td colSpan={9} style={{ textAlign:'center', padding:'3rem', color:'#7A919E' }}>Carregando...</td></tr>
                  : filtered.length===0
                    ? <tr><td colSpan={9} style={{ textAlign:'center', padding:'3rem', color:'#7A919E' }}>Nenhum lançamento encontrado</td></tr>
                    : filtered.map(l=>{
                      const parc=l.parcelas||[]; const pagas=parc.filter(p=>p.pago).length
                      const st=ST_MAP[l.status_entrega]; const tp=ST_MAP[l.tipo_pagamento]
                      return (
        ...
        <div style={s.overlay} onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div style={s.modal}>
            ...
            {detalhe?(
              <div style={{ padding:'1.25rem 1.5rem' }}>
                      ...
                      <div key={p.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', borderBottom:'1px solid #DDE5EA', background:p.pago?'#F0FFF4':'#fff' }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                          {p.pago?(
                            ...
                            <button onClick={()=>handlePagar(p.id!,detalhe.id)} disabled={!!acao} style={{ ...s.btnTeal, padding:'4px 12px', fontSize:11, opacity:!!acao?.5:1 }}>
                ...
                {detalhe.status_entrega==='pendente'&&(
                  <button onClick={()=>handleEntrega(detalhe.id)} disabled={!!acao} style={{ ...s.btnGrn, width:'100%', justifyContent:'center', opacity:!!acao?.6:1 }}>
            ...
            <div style={s.mfoot}>
              ...
              {!detalhe&&<button onClick={handleSave} disabled={saving} style={{ ...s.btnTeal, opacity:saving?.6:1 }}>{saving?'Salvando...':'Salvar'}</button>}
            </div>
          </div>
        </div>
      )}
